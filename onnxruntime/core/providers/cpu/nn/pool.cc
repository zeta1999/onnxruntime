...
Status Pool<float, MaxPool<8 /*VERSION*/>>::Compute(OpKernelContext* context) const {
  ...
  switch (kernel_shape.size()) {
    case 1: {
      ...
      RunLoop<MaxPool1DTask>(
          tp, total_channels,
          {X_data, Y_data, I_data, x_step, y_step, dilation_h, pooled_height, stride_h(), height, kernel_shape, pads});
      break;
    }
    ...
    case 3: {
      ...
      RunLoop<MaxPool3DTask>(tp, total_channels,
...
ONNX_CPU_OPERATOR_VERSIONED_KERNEL(MaxPool, 8, 9,
                                   KernelDefBuilder()
                                       .TypeConstraint("T", DataTypeImpl::GetTensorType<float>())
                                       .TypeConstraint("I", DataTypeImpl::GetTensorType<int64_t>()),
                                   Pool<float, MaxPool<8 /*VERSION*/>>);

ONNX_CPU_OPERATOR_VERSIONED_KERNEL(MaxPool, 10, 10,
                                   KernelDefBuilder()
                                       .TypeConstraint("T", DataTypeImpl::GetTensorType<float>())
                                       .TypeConstraint("I", DataTypeImpl::GetTensorType<int64_t>()),
                                   Pool<float, MaxPool<8 /*VERSION*/>>);

ONNX_CPU_OPERATOR_KERNEL(MaxPool, 11,
                         KernelDefBuilder()
                             .TypeConstraint("T", DataTypeImpl::GetTensorType<float>())
                             .TypeConstraint("I", DataTypeImpl::GetTensorType<int64_t>()),
                         Pool<float, MaxPool<8 /*VERSION*/>>);
