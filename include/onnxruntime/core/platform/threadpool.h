...
class ThreadPool {
 public:
   //TODO: replace it with our own implementation with thread affinity
  using ThreadEnvironment=Eigen::StlThreadEnvironment;

  ThreadPool(int num_threads, bool allow_spinning, ThreadEnvironment& env, EigenAllocator* allocator = nullptr)
      : impl_(num_threads, allow_spinning, env), num_threads_(num_threads), allocator_(allocator) {}
  /*
  ...
  */
  ...
  Deprecated. Please use the parallelFor function below(The first letter is lowercase)
  */
...
#pragma omp parallel for
  ...
  /////////////////////////////////////////////////////////////////////////////
  //The following code are copied from the Eigen Library
  /////////////////////////////////////////////////////////////////////////////
    ...
    EIGEN_STRONG_INLINE void memcpy(void* dst, const void* src, size_t n) const {
      ...
      if (n <= kMinBlockSize || num_threads < 2) {
        ...
        for (size_t i = 1; i < num_threads; ++i) {
          //@snnn: we have to make a const_cast here to make this class compatible with Eigen::TensorEvaluator
          const_cast<ThreadPool*>(this)->enqueue_with_barrier(&barrier, [n, i, src_ptr, dst_ptr, blocksize] {
    ...
    EIGEN_STRONG_INLINE void memcpyHostToDevice(void* dst, const void* src, size_t n)  { memcpy(dst, src, n); }
    EIGEN_STRONG_INLINE void memcpyDeviceToHost(void* dst, const void* src, size_t n)  { memcpy(dst, src, n); }
    ...
    EIGEN_STRONG_INLINE int numThreadsInPool() const { return impl_.NumThreads(); }
    ...
    EIGEN_STRONG_INLINE Notification* enqueue(Function&& f, Args&&... args)  {
      Notification* n = new Notification();
      impl_.Schedule(std::bind(&FunctionWrapperWithNotification<Function, Args...>::run, n, std::move(f), args...));
      return n;
    }

    template <class Function, class... Args>
    EIGEN_STRONG_INLINE void enqueue_with_barrier(Barrier* b, Function&& f, Args&&... args)  {
      impl_.Schedule(std::bind(&FunctionWrapperWithBarrier<Function, Args...>::run, b, std::move(f), args...));
    }

    template <class Function, class... Args>
    EIGEN_STRONG_INLINE void enqueueNoNotification(Function&& f, Args&&... args)  {
      if (sizeof...(args) > 0) {
        impl_.Schedule(std::bind(std::move(f), args...));
      } else {
        impl_.Schedule(std::move(f));
      }
    }

    // Returns a logical thread index between 0 and impl_.NumThreads() - 1 if
    // called from one of the threads in pool_. Returns -1 otherwise.
    EIGEN_STRONG_INLINE int currentThreadId() const { return impl_.CurrentThreadId(); }
    ...
    void parallelFor(Eigen::Index n, const Eigen::TensorOpCost& cost,
      ...
      handleRange = [=, &handleRange, &barrier, &f](Eigen::Index firstIdx, Eigen::Index lastIdx) {
        while (lastIdx - firstIdx > block.size) {
          ...
          impl_.Schedule([=, &handleRange]() { handleRange(midIdx, lastIdx); });
          lastIdx = midIdx;
        }
      ...
      if (block.count <= numThreads()) {
        ...
        // numThreads() threads.
        impl_.Schedule([=, &handleRange]() { handleRange(0, n); });
      }
    ...
    void parallelForAsync(Eigen::Index n, const Eigen::TensorOpCost& cost,
      ...
      ctx->handle_range = [this, ctx, block](Eigen::Index firstIdx, Eigen::Index lastIdx) {
        while (lastIdx - firstIdx > block.size) {
          // Split into halves and schedule the second half on a different thread.
          const Eigen::Index midIdx = firstIdx + Eigen::divup((lastIdx - firstIdx) / 2, block.size) * block.size;
          impl_.Schedule([ctx, midIdx, lastIdx]() { ctx->handle_range(midIdx, lastIdx); });
          lastIdx = midIdx;
        }
      ...
      if (block.count <= numThreads()) {
        ...
        // Execute the root in the thread pool to avoid running work on more than
        // numThreads() threads.
        impl_.Schedule([ctx, n]() { ctx->handle_range(0, n); });
      }
    }
    ...
    const ThreadPoolInterface* getPool() const { return &impl_; }
    ThreadPoolInterface* getPool() { return &impl_; }

    // Allocator accessor.
    EigenAllocator* allocator() const { return allocator_; }

   private:
    typedef Eigen::TensorCostModel<ThreadPool> CostModel;
   ...
   private:
    ThreadPoolTempl<ThreadEnvironment> impl_;
    int num_threads_;
